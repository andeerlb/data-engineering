...
## Notes
- Both databases were running in Docker containers, with no performance tuning.
- Data types and query logic are equivalent.
- The goal is to demonstrate performance differences for analytical (OLAP) workloads.
- avg(value) results differ due to the random number generation method, but this does not affect the performance comparison.

## Conclusion
ClickHouse is designed for massive analytical workloads and aggregated queries on large datasets, delivering much higher performance than PostgreSQL for this type of workload. PostgreSQL is excellent for OLTP and hybrid workloads, but for pure analytics at scale, ClickHouse stands out.
